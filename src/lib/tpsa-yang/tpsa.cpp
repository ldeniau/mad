...
void init_prod_index(unsigned int nv, unsigned int nd)
{
    ...
    // deallocating temporary variables -- cipri.tom 24/06/14
    for (size_t i = 0; i < nv+1; ++i)
        delete[] m[i];
    delete[] m;
...
/**\brief De-Initialize the TPSA library, preventing memory leaks.
 *
 * This must be called after finishing with the current type of TPSA (nv, nd),
 * especially if there are consequent calls to ad_init or ad_reserve
 *
 * cipri.tom 24/06/14
 */
#ifdef MSVC_DLL
extern "C" _declspec(dllexport) void _stdcall ad_fini()
#else
void ad_fini()
#endif
{
