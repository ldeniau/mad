  ...
  elseif self.fail then
    if ide then
      if all and verbose then
        io.stdout:write('>>> '..method..'\nElapsed time '..self.time..' s\nFramework failed\n')
      elseif not all then
        io.stderr:write('Failed:'..string.match(self.message,'^(.-:%d-:)\n')..method..'\n'..string.match(self.message,'^.-:%d-:(.*)'))
      end
    else
      if all and verbose then
        io.stdout:write('>>> '..method..'\nElapsed time '..self.time..' s\nFramework failed\n')
  ...
  elseif self.ok then
    ...
    if ide then
      ...
      elseif not all then
        ...
        local message = string.match(self.message,'(.-)stack traceback:.*') or self.message
        message = string.gsub(message,'(\n *actual)',', actual')
        io.stderr:write('Failed:'..string.match(message,'^(.-:%d+:)')..' '..method..string.match(message,'^.-:%d+:(.*)')..'\n')
      end
    else
