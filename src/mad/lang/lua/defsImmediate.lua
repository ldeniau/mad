    ...
    if loc == '' then
        ...
        while ofs < defs._maxPos do
            ...
            if a then
                ofs = a + 1
                line = line + 1
                if ofs < defs._lastPos then lasttokline = line end
            else
                break
            end
        end
        error("Unexpected token '"..lasttok.."' on line "..tostring(lasttokline))
    end
end
